...
- [ ] User authenticates with Google via in-extension OAuth (chrome.identity) on first use
- [ ] Extension parses the PDF and sends it to Gemini API (authenticated via Google OAuth) to extract structured data and pre-generate answers to all common application questions
...
- **AI integration:** Gemini API via in-extension Google OAuth (chrome.identity) — user signs in with Google once; no API key paste required
...
- **Auth:** In-extension Google OAuth via chrome.identity — extensions are sandboxed and cannot read gemini-cli's filesystem credentials
...
| Google OAuth via chrome.identity | Extensions can't read filesystem; chrome.identity is the standard MV3 auth path — user signs in once | — Pending |
...
---
*Last updated: 2026-03-22 after research — auth approach corrected from gemini-cli to chrome.identity Google OAuth*
