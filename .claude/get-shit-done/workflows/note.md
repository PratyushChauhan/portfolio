...
- **Global scope**: `/home/cipher/dev/portfolio/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/cipher/dev/portfolio/.claude/notes/`)
...
2. Glob `/home/cipher/dev/portfolio/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/cipher/dev/portfolio/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/cipher/dev/portfolio/.claude/notes/` — works in any directory
